...
db = DataBank()
db.LoadDB("./db.sdb")
print("[DB] Loaded " + str(len(db.starObjs)) + " entries")
...
for i in range(1, STAR_COUNT * STEP + 1, STEP):
    id = "HD " + str(i)
    if not db.HasStar(id):
        db.IDRequest(id)
        newStars += 1
        if (newStars % 100) == 0: 
            print(str(newStars) + " new Stars added!")
            db.SaveDB()
            print("[DB] Saved " + str(len(db.starObjs)) + " points")
...
plt.title("Star-Plotting of " + str(len(plotPoints[0])) + " stars") 
